frappe.ui.form.on("Stock Reconciliation", {
	refresh(frm) {
		if (frm.doc.docstatus < 1) {
			// Add custom button to fetch items with Item Group support
			frm.add_custom_button(__("Fetch Items by Item Group"), function () {
				prompt_managely_fetch_items(frm);
			});

			// Override standard get_items event as well
...
function prompt_managely_fetch_items(frm) {
	let fields = [
		{
			label: __("Warehouse"),
			fieldname: "warehouse",
	...
	frappe.prompt(
		...
		function (data) {
			frappe.call({
				...
				freeze: true,
				freeze_message: __("Fetching items from warehouse..."),
				callback: function (r) {
