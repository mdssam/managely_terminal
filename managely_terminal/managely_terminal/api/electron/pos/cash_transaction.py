...
def get_cash_io_config(pos_profile=None):
    ...
    allowed_modes_raw = []
    if pos_profile and pos_profile != _SYNTHETIC_PROFILE:
    ...
    if not allowed_modes_raw:
        all_cash = frappe.get_all("Mode of Payment", filters={"type": "Cash"}, fields=["name"])
        allowed_modes_raw = [{"mode_of_payment": m.name} for m in all_cash]
        if not allowed_modes_raw:
            allowed_modes_raw = [{"mode_of_payment": "Cash"}]
    ...
    for am in allowed_modes_raw:
        mop = am.get("mode_of_payment")
        custom_curr = am.get("custom_currency")
        currency = custom_curr or company_currency
        if company and not custom_curr:
...
def create_cash_transaction(transaction_type, amount, description="",
    ...
    try:
        ...
        from managely_terminal.managely_terminal.doctype.pos_suspended_transaction.pos_suspended_transaction import (
